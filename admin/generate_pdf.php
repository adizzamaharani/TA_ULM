...
// --- QR CODE URL PENDEK ---
// Gunakan URL pendek agar QR Code tidak terlalu padat dan bisa discan HP
$secret_key = 'ULM_FAPERTA_2025_SECRET';
$full_token = hash('sha256', $surat['id'] . $surat['nim'] . $surat['jenis_surat'] . $secret_key);
...
$qr_content = $base_url . '/verifikasi_qr.php?type=surat&id=' . $surat['id'] . '&t=' . $short_token;

$qr_temp_file = tempnam(sys_get_temp_dir(), 'qr_') . '.png';
$old_er = error_reporting(E_ALL & ~E_DEPRECATED);
QRcode::png($qr_content, $qr_temp_file, QR_ECLEVEL_M, 6, 2);
error_reporting($old_er);

if (file_exists($qr_temp_file)) {
    $pdf->Image($qr_temp_file, 105, $pdf->GetY() + 1, 30);
...
// Output PDF
ob_end_clean();
$pdf->Output('I', 'Surat_' . str_replace(' ', '_', $surat['jenis_surat']) . '_' . $surat['nim'] . '.pdf');
