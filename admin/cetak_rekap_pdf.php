...
switch ($jenis_laporan) {
    ...
    case 'prodi':
        ...
        $pdf->SetFont('Times', 'B', 10);
        $pdf->Cell(15, 7, 'No', 1, 0, 'C');
        $pdf->Cell(115, 7, 'Program Studi', 1, 0);
        $pdf->Cell(60, 7, 'Jumlah', 1, 1, 'C');
        $pdf->SetFont('Times', '', 10);
        $rekap_prodi = [];
    ...
    case 'jenis_surat':
        ...
        $pdf->SetFont('Times', 'B', 10);
        $pdf->Cell(15, 7, 'No', 1, 0, 'C');
        $pdf->Cell(115, 7, 'Jenis Surat', 1, 0);
        $pdf->Cell(60, 7, 'Jumlah', 1, 1, 'C');
        $pdf->SetFont('Times', '', 10);
        $rekap_jenis_surat = [];
    ...
    case 'tanggal':
        ...
        $pdf->SetFont('Times', 'B', 10);
        $pdf->Cell(15, 7, 'No', 1, 0, 'C');
        $pdf->Cell(115, 7, 'Tanggal Pengajuan', 1, 0);
        $pdf->Cell(60, 7, 'Jumlah', 1, 1, 'C');
        $pdf->SetFont('Times', '', 10);
        $rekap_per_tanggal = [];
        ...
        foreach ($rekap_per_tanggal as $tanggal => $jumlah) {
            $pdf->Cell(15, 7, $no++, 1, 0, 'C');
            $pdf->Cell(115, 7, $tanggal, 1, 0);
            $pdf->Cell(60, 7, $jumlah, 1, 1, 'C');
        }

        // Tambahan Tabel Detail Pengajuan per Tanggal
        $pdf->Ln(10);
        $pdf->SetFont('Times', 'B', 11);
        $pdf->Cell(0, 7, 'Detail Pengajuan per Tanggal', 0, 1, 'L');
        $pdf->Ln(2);

        $pdf->SetFont('Times', 'B', 10);
        $pdf->Cell(10, 7, 'No', 1, 0, 'C');
        $pdf->Cell(30, 7, 'Tgl Pengajuan', 1, 0, 'C');
        $pdf->Cell(50, 7, 'Nama / NIM', 1, 0, 'C');
        $pdf->Cell(50, 7, 'Program Studi', 1, 0, 'C');
        $pdf->Cell(50, 7, 'Jenis Surat', 1, 1, 'C');

        $pdf->SetFont('Times', '', 9);
        if (empty($rekap_surat)) {
            $pdf->Cell(190, 10, 'Tidak ada data pengajuan pada periode ini.', 1, 1, 'C');
        } else {
            $no_detail = 1;
            foreach ($rekap_surat as $surat) {
                $pdf->Cell(10, 10, $no_detail++, 1, 0, 'C');
                $pdf->Cell(30, 10, date('d-m-Y, H:i', strtotime($surat['tanggal_pengajuan'])), 1, 0, 'C');

                // MultiCell untuk Nama dan NIM
                $x = $pdf->GetX();
                $y = $pdf->GetY();
                $pdf->MultiCell(50, 5, htmlspecialchars_decode($surat['nama_lengkap']) . "\n" . htmlspecialchars_decode($surat['nim']), 1, 'L');
                $pdf->SetXY($x + 50, $y);

                $pdf->Cell(50, 10, htmlspecialchars_decode($surat['nama_prodi']), 1, 0);
                $pdf->Cell(50, 10, htmlspecialchars_decode($surat['jenis_surat']), 1, 1);
            }
        }
        break;
    ...
    default:
        ...
        $pdf->Cell(10, 7, 'No', 1, 0, 'C');
        $pdf->Cell(30, 7, 'Tgl Pengajuan', 1, 0, 'C');
        $pdf->Cell(55, 7, 'Nama / NIM', 1, 0, 'C');
        $pdf->Cell(60, 7, 'Program Studi', 1, 0, 'C');
        $pdf->Cell(55, 7, 'Jenis Surat', 1, 0, 'C');
        $pdf->Cell(25, 7, 'Status', 1, 0, 'C');
        $pdf->Cell(35, 7, 'File SK', 1, 1, 'C');
        ...
        if (empty($rekap_surat)) {
            ...
            foreach ($rekap_surat as $surat) {
                ...
                $pdf->Cell(30, 10, date('d-m-Y, H:i', strtotime($surat['tanggal_pengajuan'])), 1, 0);
                
                // MultiCell untuk Nama dan NIM
                $x = $pdf->GetX();
                $y = $pdf->GetY();
                $pdf->MultiCell(55, 5, htmlspecialchars_decode($surat['nama_lengkap']) . "\n" . htmlspecialchars_decode($surat['nim']), 1, 'L');
                $pdf->SetXY($x + 55, $y);

                $pdf->Cell(60, 10, htmlspecialchars_decode($surat['nama_prodi']), 1, 0);
                $pdf->Cell(55, 10, htmlspecialchars_decode($surat['jenis_surat']), 1, 0);
                $pdf->Cell(25, 10, htmlspecialchars_decode($surat['status']), 1, 0, 'C');
                
                // Link ke PDF Surat Asli
                $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
                $base_url = $protocol . $_SERVER['HTTP_HOST'] . '/TA_ULM';
                $link_surat = $base_url . '/admin/generate_pdf.php?id=' . $surat['id'];
                
                $pdf->SetTextColor(0, 0, 255);
                $pdf->SetFont('', 'U');
                $pdf->Cell(35, 10, 'Klik untuk Buka', 1, 1, 'C', false, $link_surat);
...
// TTD Digital berupa QR Code Dinamis
$qr_temp_dir = '../assets/uploads/qr/';
if (!file_exists($qr_temp_dir)) {
    mkdir($qr_temp_dir, 0777, true);
}
$qr_filename = $qr_temp_dir . 'qr_rekap_' . time() . '.png';

$aes_key = 'KUNCI_RAHASIA_SKRIPSI_FAPERTA_26';
$aes_iv = '1234567890123456';
$secret_key = 'ULM_FAPERTA_2025_SECRET';
$token = hash('sha256', 'Rekap' . $periode_teks . $secret_key);
...
$encrypted_data = openssl_encrypt($json_payload, 'aes-256-cbc', $aes_key, 0, $aes_iv);

$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
$base_url = $protocol . $_SERVER['HTTP_HOST'] . '/TA_ULM';
$verify_url = $base_url . '/verifikasi_qr.php?data=' . urlencode(base64_encode($encrypted_data));

QRcode::png($verify_url, $qr_filename, QR_ECLEVEL_L, 4, 1);

$pdf->Image($qr_filename, $x_pos + 5, $pdf->GetY() + 1, 25);
$pdf->Ln(25);
